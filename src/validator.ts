...
const checkUnknownMetrics = (metricsMap: Map<string, string>, knownMetrics?: Metrics): void => {
...
const checkMandatoryMetrics = (metricsMap: Map<string, string>, metrics: Metrics = baseMetrics): void => {
...
const checkMetricsValues = (metricsMap: Map<string, string>, metrics: Metrics, metricsValues: AllMetricValues): void => {
  metrics.forEach((metric: Metric) => {
    ...
    if (!metricsValues[metric].includes(userValue as MetricValue)) {
      const allowedValuesHumanized = metricsValues[metric]
        .map((value: MetricValue) => `${value} (${humanizeBaseMetricValue(value, metric)})`)
        .join(', ');
      throw new Error(
        `Invalid value for CVSS metric ${metric} (${humanizeBaseMetric(metric)})${
          userValue ? `: ${userValue}` : ''
...
export const validate = (cvssStr: string): ValidationResult => {
  ...
  const isTemporal = [...metricsMap.keys()].some((metric) => temporalMetrics.includes(metric as TemporalMetric));
  const isEnvironmental = [...metricsMap.keys()].some((metric) => environmentalMetrics.includes(metric as EnvironmentalMetric));
